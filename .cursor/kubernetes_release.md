## kubernetes release 展示形式

kubernetes release 内容如下：

```
See kubernetes-announce@. Additional binary downloads are linked in the CHANGELOG[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.34.md].

See the CHANGELOG[https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.34.md] for more details.
```

与 containerd 实际内容写在 release 中不同，kubernetes release 的实际内容写在对应版本的 CHANGELOG 中，例如 v1.34.0、v1.34.1、v1.34.0-rc.2 等等都在 CHANGELOG-1.34.md 中

以 CHANGELOG-1.34.md 为例，开头的目录格式如下：
...
以 v1.34.0 为例，主要关注其中 Changelog since v1.33.0、Urgent Upgrade Notes、Changes by Kind 的变更，内容参考如下：
...
对于上述内容中每一个变更，都会有对于的 pull request 信息和 作者信息，其中的 pull request 和 containerd 不同，不会是 cherrypick 的 pr，因此不需要判断是否是 cherrypick，可以直接读取 pull request 内容，以及 pull request 中对应关联的 issue 内容（可能会有多个）